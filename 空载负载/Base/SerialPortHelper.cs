...
namespace 空载负载.Base
{
        ...
        private void DataReceivedHandler(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            if (cuucentcommand == 5)
            {
                if (readlength >= 10)
                {
                    ...
                    byte[] arraySet = { 0x52, 0x53, 0x03, 0x02, 0x4F, 0x4B, 0x9F, 0x00, 0x45, 0x44 }; 
                    int lengthToCompare = 10; // 比较前10个字节
                    bool areEqual = arraySource.Take(lengthToCompare).SequenceEqual(arraySet.Take(lengthToCompare));
                    if (areEqual)
                    {
                        MessageInit myMessage = new MessageInit();
                        myMessage.Message = "PortMessage_Set";
                        WeakReferenceMessenger.Default.Send(myMessage);
                        readlength = 0;
                        Array.Clear(_buffer, 0, 500);
                        cuucentcommand = 0;
    ...
    {
        byte[] dehead = {0x52 ,0x53, 0x03, 0x02, 0x28, 0x00};
        byte[] deheadcmc = {0x03, 0x02, 0x28, 0x00 };
        byte[] destination = new byte[40];
        byte[] destinationall = new byte[46];
        public byte[] destinationallcac { get; set; } = new byte[50];

        public void Conversion()
        {

            byte[] source = BitConverter.GetBytes(Highpressure);
            Array.ConstrainedCopy(source, 0, destination, 0, 4);            
            source = BitConverter.GetBytes(Highcurrent);
            Array.ConstrainedCopy(source, 0, destination, 4, 4);
            source = BitConverter.GetBytes(Lowpressure);
            Array.ConstrainedCopy(source, 0, destination, 8, 4);
            source = BitConverter.GetBytes(Lowcurrent);
            Array.ConstrainedCopy(source, 0, destination, 12, 4);
            source = BitConverter.GetBytes(gaoyacezhizu);
            Array.ConstrainedCopy(source, 0, destination, 16, 4);
            source = BitConverter.GetBytes(diyacezhizu);
            Array.ConstrainedCopy(source, 0, destination, 20, 4);
            source = BitConverter.GetBytes(Data7);
            Array.ConstrainedCopy(source, 0, destination, 24, 4);
            source = BitConverter.GetBytes(Dianliudata);
            Array.ConstrainedCopy(source, 0, destination, 28, 4);

            destination[32] = PhaseNumber;
            destination[33] = selectbiaofa;

            byte[] bytes = BitConverter.GetBytes(ProductCapacity);
            destination[34] = bytes[0];
            destination[35] = bytes[1];

            destination[36] = zhizuwendu;
            destination[37] = shipinwendu;

            bytes = BitConverter.GetBytes(mubiaowendu);

            destination[38] = bytes[0];
            destination[39] = bytes[1];

            Array.ConstrainedCopy(deheadcmc, 0, destinationall, 0, 4);
            Array.ConstrainedCopy(destination, 0, destinationall, 4, 40);
            ushort cmc = MyTools.CRCCalc2(destinationall);

            Array.ConstrainedCopy(dehead, 0, destinationall, 0, 6);
            Array.ConstrainedCopy(destination, 0, destinationall, 6, 40);
            
            Array.ConstrainedCopy(destinationall, 0, destinationallcac, 0, 46);

            bytes = BitConverter.GetBytes(cmc);
            destinationallcac[46] = bytes[0];
            destinationallcac[47] = bytes[1];

            destinationallcac[48] = 0x45;
            destinationallcac[49] = 0x44;

        }

        public float Highpressure;  //高压侧额定电压，KV   
        public float Highcurrent;  //高压侧额定电流，A
        public float Lowpressure;   //低压侧额定电压，V
        public float Lowcurrent;   //低压侧额定电流，A
        public float gaoyacezhizu;   //高压侧直阻，Ω
        public float diyacezhizu;   //低压侧直阻，mΩ
        public float Data7;   //电压互感器变比
        public float Dianliudata;   //电流互感器变比
        public byte PhaseNumber;     //0代表单相变压器试验，非零代表三相变压器试验
        public byte selectbiaofa;    //为0代表双表法试验，0x0f 三表法显示线电压，0xf0三表法显示相电压
        public ushort ProductCapacity;  //代表变压器容量，KVA, 容量为无符号16位值表示，P3为低字节，P4为高字节
        public byte zhizuwendu;     //代表测直阻时的温度，8位整型数，范围为-128~127摄氏度
        public byte shipinwendu;     //代表现在的试品温度，8位整型数，范围为-128~127摄氏度；
        public ushort mubiaowendu;    //代表校正的目标温度，16为整型数，单位为摄氏度。P7为低字节，P8高字节。
