...
{
    public class Load : ObservableObject
    {
        ...
        //试验电流
        public float TextCurrent { get; set; }

        //保护电压..
        public float ProtectVoltage { get; set; }


        private float protectCurrent;
        //保护电流..

        public float ProtectCurrent { get => protectCurrent; set { SetProperty(ref protectCurrent, value); } }

        //操作模式.
        public int IsAutoOrMu { get; set; } = 0;
        ...
        private Table_LoadStandardInfo m_sampleinLoad { get; set; }

        private UserLoadRecordModel m_UserLoadRecordModel { get; set; }
        private UserLoadHeadModel m_UserLoadHeadModel { get; set; }
        ...
        //第2....
        public void Initiate(Table_ProductInfo table_ProductInfo, Table_LoadStandardInfo table_LoadStandardInfo)
        {
            m_sampleinformation = table_ProductInfo;
            m_sampleinLoad = table_LoadStandardInfo;

            m_UserLoadHeadModel.ProductType = m_sampleinLoad.ProductType;
            m_UserLoadHeadModel.ProductTuhao = m_sampleinLoad.ProductTuhao;
            m_UserLoadHeadModel.Loadloss = m_sampleinLoad.Loadloss;
            m_UserLoadHeadModel.LoadMainReactance = m_sampleinLoad.LoadMainReactance;
        ...
        //第一
        public void InitiateModel(UserLoadRecordModel userLoadRecordModel,
                                  UserLoadHeadModel userLoadStandardModel,
                                  UserLoadMainModel UserLoadMainModel,
                                  UserCommunicationModel userCommunicationModel,
                                  PlcStateModel plcStateModel)
        {
            m_UserLoadRecordModel = userLoadRecordModel;
            m_UserLoadHeadModel = userLoadStandardModel;
            m_UserLoadMainModel = UserLoadMainModel;
        ...
        public Load()
        {
            WeakReferenceMessenger.Default.Register<MessageInit>(this, (r, user) =>
            {
                ...
                {
                    m_UserLoadMainModel.Umn_ab = user.Data.fltVoltage[0];
                    m_UserLoadMainModel.Urms_ab = user.Data.fltMeanVoltage[0];
                    m_UserLoadMainModel.Irms_ab = user.Data.fltCurrent[0];
                    m_UserLoadMainModel.P_ab = user.Data.fltPower[0];

                    m_UserLoadMainModel.Umn_bc = user.Data.fltVoltage[1];
                    m_UserLoadMainModel.Urms_bc = user.Data.fltMeanVoltage[1];
                    m_UserLoadMainModel.Irms_bc = user.Data.fltCurrent[1];
                    m_UserLoadMainModel.P_bc = user.Data.fltPower[1];

                    m_UserLoadMainModel.Umn_ca = user.Data.fltVoltage[2];
                    m_UserLoadMainModel.Urms_ca = user.Data.fltMeanVoltage[2];
                    m_UserLoadMainModel.Irms_ca = user.Data.fltCurrent[2];
                    m_UserLoadMainModel.P_ca = user.Data.fltPower[2];
                    ...
                    m_UserLoadMainModel.Urms_vc = user.Data.fltAverageVoltage;
                    m_UserLoadMainModel.Irms_ic = user.Data.fltAverageCurrent;
                    m_UserLoadMainModel.Is_pv = user.Data.fltFreq;
                    m_UserLoadMainModel.Pow_loss = user.Data.fltMeasureSumPower;

                    m_UserLoadMainModel.flt_UkIoPercent = user.Data.fltUkIoPercent;

                    m_UserLoadMainModel.flt_CosQ = user.Data.fltCosQ;

                    ////////////////////////////////////////////////////////////////////////
                    //////////////////////////////////////////////////////////////////////////               
                    m_UserLoadRecordModel.ATPVoltage = user.Data.fltAverageVoltage;
                    m_UserLoadRecordModel.RMSvalue = user.Data.fltAverageCurrent;
                    m_UserLoadRecordModel.IRMSvalue = user.Data.fltCosQ;
                    m_UserLoadRecordModel.FHzSvalue = user.Data.fltFreq;
                    
                    
                    //这个就是zk75
                    m_UserLoadRecordModel.FltUkIoPercent = user.Data.fltUkt;
                    m_UserLoadRecordModel.FltUkIoPercent = (float)Math.Round(m_UserLoadRecordModel.FltUkIoPercent, 4); // 保留4位小数;                    

                    //这个就是 pk75
                    m_UserLoadRecordModel.FltUkt = user.Data.fltTempAdjustPower[1]; //1温度系数法、2国标公式
                    m_UserLoadRecordModel.FltUkt = (float)Math.Round(m_UserLoadRecordModel.FltUkt, 4); // 保留4位小数;                    

                    //这个就是 zt
                    m_UserLoadRecordModel.FltZt = user.Data.fltZt;
                    m_UserLoadRecordModel.FltZt = (float)Math.Round(m_UserLoadRecordModel.FltZt, 3); // 保留4位小数;                    


                    //合格
                    float loss, lossup;
                    float.TryParse(m_UserLoadHeadModel.Loadloss, out loss);
                    float.TryParse(m_UserLoadHeadModel.LoadlossUpperimit, out lossup);
                    loss = loss + (loss / lossup);

                    float losscurrent, losslosscurrentup;
                    float.TryParse(m_UserLoadHeadModel.LoadMainReactance, out losscurrent);
                    float.TryParse(m_UserLoadHeadModel.LoadMainReactanceUpperrimit, out losslosscurrentup);
        ...
        private void DoCommandAuto(object param)         
        {
            ...
            Task task = new Task(async () =>
            {
                ...
                if (TextCurrent == 0)
                {
                    MessageBox.Show("错误,试验电流为0");
                    return;
                }


                //ProtectCurrent = (float)(TextCurrent * 1.2);

                if (ProtectVoltage == 0)
                {
                    MessageBox.Show("错误,保护电压为0");
                    return;
                }
                ...
                while (true)
                {
                    ...
                    //平均电流..                    
                    if (m_UserLoadMainModel.Irms_ic >= TextCurrent)
                    {
                        AddRecord("电流到达标准,停止升压...");
                        //这里强制停止升压..
                        plc200.Write("M12.1", false);
                        break;
                    }
        ...
        bool SaveRecord()
        {
            Table_LoadRocreadInfo temp = new Table_LoadRocreadInfo();
            ...
            temp.Loadloss = m_UserLoadRecordModel.FltUkt.ToString();
            temp.LoadMainReactance = m_UserLoadRecordModel.FltUkIoPercent.ToString();
            ...
            try
            {
                ...
                {
                    context.LoadRocreadInfo.Add(temp);
        ...
        public void InitializationManager()
        {
            ...
            if (waitWindow == null)
            {
                ...
                waitWindow.SetText("请等待,正在负载初始化..");
